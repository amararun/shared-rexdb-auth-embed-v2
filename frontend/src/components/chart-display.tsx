...
export const ChartDisplay: React.FC<ChartDisplayProps> = ({ charts }) => {
  ...
  console.log('ChartDisplay rendered with charts:', {
    totalCharts: charts.length,
    currentIndex,
    chartUrls: charts.map(c => c.url),
    chartSources: charts.map(c => c.source)
  });

  const goToPrevious = () => {
    const newIndex = currentIndex === 0 ? charts.length - 1 : currentIndex - 1;
    console.log('Navigating to previous chart:', { currentIndex, newIndex });
    setCurrentIndex(newIndex);
  };

  const goToNext = () => {
    const newIndex = currentIndex === charts.length - 1 ? 0 : currentIndex + 1;
    console.log('Navigating to next chart:', { currentIndex, newIndex });
    setCurrentIndex(newIndex);
  };
            ...
            <div className="absolute bottom-2 left-0 right-0 flex justify-center gap-1">
              {charts.map((_, index) => (
                <button
                  key={index}
                  onClick={() => {
                    console.log('Navigating to chart via dot:', { fromIndex: currentIndex, toIndex: index });
